...
class Auth extends CI_Controller
{
    ...
    { 
        ...
        if($this->form_validation->run() == false) {
            ...
            Congratulations,your account has been creted. please Activate your Account </div>');
            redirect('auth');
        }
    }
    ...
    private function _sendEmail($token, $type)
    {
        ...
        $config['smtp_user'] = 'hardiantisiti567@gmail.com';
        $config['smtp_pass'] = 'kblyovbvzacswzwb';
        $config['smtp_port'] = 465;
        $config['mailtype'] = 'html';
        $config['charset'] = 'utf-8';
        $this->email->initialize($config);
        $this->email->set_newline("\r\n");
        $this->email->from('hardiantisiti567@gmail.com', 'Web Programing SMKN8Jember');
        $this->email->to($this->input->post('email'));

        if($type == 'verify') {
            $this->email->subject('Account Verivication');
            $this->email->message('Clik this link to actived your account:
        ...
        if ($user) {
            ...
            if ($user_token) {
                if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
                    ...
                    . $email . 'has been Activited!please login</div>');
                } else {
    ...
    public function changePassword()
    {
     ...
     public function forgotPassword()
     {
        ...
        if ($this->form_validation->run()== false) {
            ...
            $email = $this->input->post('email');
            $user = $this->db->get_where('user', ['email' => $email, 'is_active' => 11]);
